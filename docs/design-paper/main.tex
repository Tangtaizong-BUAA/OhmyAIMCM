...
\hypersetup{unicode=true,colorlinks=true,linkcolor=black,citecolor=black,urlcolor=black,
  pdftitle={OhmyAIMCM：面向数学建模论文的双路论文蒸馏与可追溯数模工作流},
  pdfauthor={OhmyAIMCM 项目设计说明},pdfsubject={架构、信息流、写作蒸馏、科学绘图与验证边界}}
...
\fancyhead[L]{\small OhmyAIMCM：论文蒸馏与工作流}
\fancyhead[R]{\small 技术设计论文}
\fancyfoot[C]{\small\thepage}
\renewcommand{\headrulewidth}{.3pt}
\newcolumntype{Y}{>{\raggedright\arraybackslash}X}
\newcommand{\sys}{\textsf{OhmyAIMCM}}
\newcommand{\code}[1]{\texttt{\small #1}}
\title{\bfseries OhmyAIMCM：面向数学建模论文的\\双路论文蒸馏与可追溯数模工作流}
...
本文使用 2026 年 9 月 10 日完成统一归并的资源，并于 2026 年 9 月 13 日将项目及模块命名统一为 OhmyAIMCM。主入口为 \code{OhmyAIMCM}，十九个子模块使用 \code{OhmyAIMCM-*}。当前可安装资源位于仓库的 \code{skills/}；安装器依据 manifest 校验完整模块集合。命名迁移检查与前序科学验证分别记录。
...
\begin{table}[htbp]
\centering\small
\caption{完整套件的职责划分。模块名省略共同的 \texttt{OhmyAIMCM-} 前缀；主入口为 \texttt{OhmyAIMCM}。}
\label{tab:modules}
\begin{tabularx}{\linewidth}{@{}p{24mm}Yp{12mm}Y@{}}
\toprule
层次 & 模块 & 数量 & 主要责任\\
\midrule
主协调 & OhmyAIMCM & 1 & 确认范围、按需分派、整合结果与安排复核\\
...
统一版从维护源的明确清单构建 20 个模块，映射主入口及全部子模块名称，同步相对引用和资源哈希。\code{manifest.json} 保存目标文件与源树身份，安装器 \code{install.py} 完整复制并核验资源，遇到已有同名目录时拒绝合并覆盖。它没有把原始论文、账户认证、外部运行环境或模型权重加入运行时包。
...
更新旧统一版时需要保留原整套模块及对应 manifest，再完整安装新版本。维护源和分发包承担不同责任：前者用于修改与构建，后者用于安装已确定的资源。不应递归搜集仓库中所有 SKILL.md 组成安装包，也不应从其他版本或第三方目录拼接模块。
...
本文在 2026 年 9 月 7 日的前序整体设计论文基础上修订，绑定 2026 年 9 月 10 日的 unified 发行资源。新增内容依据本地实现、归并记录、布局策略和作者提供的药材论文。外部参考网页在此前正文修订时查看，正文仅沿用相应的项目归因或规范来源。
...
\bibitem{coordinator} OhmyAIMCM 项目. Coordinator、Lightweight handoffs 与 Evidence relationships[CP]. 2026-09-10 发行资源。
\bibitem{writing} OhmyAIMCM 项目. 写作入口、通用技巧与结构化表达库[CP]. 2026-09-10 发行资源。
...
\bibitem{layout} OhmyAIMCM 项目. Aesthetic profile policy[CP]. 2026-09-10 发行资源。
\bibitem{install} 项目开发记录. 完整发行与安装核验[R]. 2026-09-10 统一记录及 2026-09-13 命名迁移核查。
...
\bibitem{query} OhmyAIMCM 项目. 条件写作技巧检索与 query\_variants.py[CP]. 2026-09-10 发行资源。
\bibitem{conflicts} OhmyAIMCM 项目. 跨来源写作经验的冲突裁决[CP]. 2026-09-10 发行资源。
...
项目名称与调用名称的对应关系见表\ref{tab:names}。当前仓库在 skills 目录维护可安装资源，文稿位于 docs/design-paper。此次命名迁移同步更新模块元数据、交叉引用与安装清单，历史科学验证按原记录范围引用。
...
对象 & 名称或责任\\\midrule
对外项目 & \code{OhmyAIMCM}\\
运行主入口 & \code{OhmyAIMCM}\\
同套子模块 & \code{OhmyAIMCM-*}，共十九个\\
当前资源 & \path{skills/}\\
发行目录 & \code{OhmyAIMCM-20260913}；含 installer、manifest 与 skills\\
...
python3 -B install.py install --bundle . --dest /target/skills\\
python3 -B install.py verify --bundle . --dest /target/skills
\end{quote}

在 \path{skills/OhmyAIMCM-writing/} 中，可查询具体写作困难：
\begin{quote}\small\ttfamily
...
前序设计论文 & 见 \path{qa.json} 中的前序提交与来源哈希记录\\[4pt]
...
当前安装说明 & 根目录 \path{README.md}、\path{install.py} 与 \path{manifest.json}\\[4pt]
...
写作规则 & \path{skills/OhmyAIMCM-writing/references/exposition-cards.json}；同目录 \path{technique-variants.json} 与 \path{conflict-decisions.md}\\[4pt]
版式策略 & \path{skills/OhmyAIMCM-typesetter/references/aesthetic-policy.md}；同目录 \path{award-layout-calibration.md}\\[4pt]
