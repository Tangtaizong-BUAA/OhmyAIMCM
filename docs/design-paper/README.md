...
[阅读论文](OhmyAIMCM-整体设计论文.pdf) · [LaTeX 源码](main.tex) · [修订说明](REVISION.md) · [核查记录](qa.json)

《OhmyAIMCM：面向数学建模论文的双路论文蒸馏与可追溯数模工作流》

2026-09-13 修订，21 页，LaTeX 源码含约 1.5 万汉字；4 幅图、5 张编号表、1 张来源索引长表、21 条参考文献。基于前序设计论文扩写，当前命名版本为 `OhmyAIMCM-20260913`；Skill 入口为 `$OhmyAIMCM`。
...
本次完成编译、21 页目视检查、发行资源哈希、完整安装和写作查询检查，37 项排版回归全部通过。文中 128 项回归、前序协作试验和固定证据改稿均引自已有记录，没有在本次重跑；没有新增模型性能实验，也未对药材源码做全部复算。原设计论文的独立审阅只适用于旧版本。本次使用 humanizer 整理文字，由同一作者代理自校。
