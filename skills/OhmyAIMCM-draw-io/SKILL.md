---
name: OhmyAIMCM-draw-io
description: OhmyAIMCM CUMCM paper suite. Create, edit, export, and review draw.io diagrams. Use for native .drawio XML generation, PNG/SVG/PDF export, SVG overlap, border-overlap, label-intrusion, label-rect, short-terminal, text-contrast, text-emphasis, and text-overflow linting, layout adjustment, and AWS icon usage.
---
...
- Codex on Windows: junction `C:\Users\YOUR_NAME\.codex\skills\OhmyAIMCM-draw-io -> D:\Prj\draw-io-skill`
...
2. For CUMCM or scientific-paper figures, read `../OhmyAIMCM-scientific-figure-maker/references/diagram-design.md` and use its paper-specific semantic and visual defaults. For other diagrams without a user-specified style, start from `references/aesthetic-templates.md` and its polished technical light template.
...
For new diagrams, choose a visual treatment before writing the XML. Scientific-paper diagrams follow `../OhmyAIMCM-scientific-figure-maker/references/diagram-design.md`: white or paper-transparent backgrounds, neutral lines, and ordinary rectangles or domain-native symbols when appropriate. Do not add rounded cards, accent colors, title/body chips, or dashboard panels merely to appear polished. User-specified and scientifically meaningful encodings remain allowed.

The following polished technical light baseline is for non-paper diagrams without a specified style, using `references/aesthetic-templates.md`.
...
For non-paper presentation diagrams, revise a visually flat hierarchy when needed. A deliberately plain scientific figure is not a defect: judge its information structure, semantic accuracy, routing, and readability at final size, not decorative richness.
...
For Chinese paper diagrams, resolve an installed regular sans-serif with Chinese
glyph coverage and set it explicitly in both the model and label styles. Follow
the shared diagram guide; do not copy a Japanese font or manuscript Songti heading
font without checking the actual rendered face. Inspect exported labels at final size.
...
- use white or paper-transparent backgrounds for paper figures; prefer transparent exports for other contexts where appropriate
...
- [ ] paper figures follow `diagram-design.md`; other unspecified diagrams use `references/aesthetic-templates.md`
...
- [ ] the background is appropriate to the paper or presentation, without decorative color blocks
