...
def copy_runtime_assets(output_dir: Path) -> None:
    for name in ("cumcm-paper.cls", "cumcm-style.sty", "semantic.lua"):
...
def render_config_tex(config: dict[str, Any]) -> str:
    ...
    abstract_switch = (
        "\\cumcmabstractpagebreaktrue" if style["abstract_page_break"] else "\\cumcmabstractpagebreakfalse"
    )
    return (
        ...
        f"\\cumcmApplyNumbering{{{style['section_numbering_style']}}}\n"
        "\\cumcmApplyCaptionStyle"
...
def render_tex(
    ...
    *,
    ...
    command = [
            ...
            "--to=latex",
            "--standalone",
            "--variable=documentclass:cumcm-paper",
...
def add_content_lock_markers(tex_path: Path, source_hash: str, rendered_hash: str) -> None:
    ...
    marker = (
        "% Generated by lab-ultra-typesetter; editable TeX.\n"
...
def _page_texts(pdf_path: Path) -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="cumcm-text-") as tmp:
...
def _bbox_pages(pdf_path: Path) -> list[dict[str, Any]]:
    require_tool("pdftotext")
    with tempfile.TemporaryDirectory(prefix="cumcm-bbox-") as tmp:
