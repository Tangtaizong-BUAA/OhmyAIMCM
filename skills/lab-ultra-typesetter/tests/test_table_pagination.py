...
def build_fixture(output: Path, *, guarded: bool, full: bool) -> Path:
    ...
    if not guarded:
        header += "\n\\makeatletter\\let\\lab-ultra@guardCompletedTable\\relax\\makeatother\n"
    header_path = output / "render-config.tex"
    header_path.write_text(header, encoding="utf-8")
    for name in ("cumcm-paper.cls", "cumcm-style.sty"):
        shutil.copy2(SKILL_DIR / "assets/latex" / name, output / name)
    run_checked(
        ["pandoc", str(ast_path), "-f", "json", "-t", "latex", "-s",
         "-V", "documentclass:cumcm-paper", f"--include-in-header={header_path}",
...
class TablePaginationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.tmp = tempfile.TemporaryDirectory(prefix="cumcm-table-regression-")
