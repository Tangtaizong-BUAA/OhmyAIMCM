...
class ConfigTests(unittest.TestCase):
    ...
    def test_canonical_style_tokens_drive_advanced_controls(self) -> None:
        ...
        self.assertIn("\\OhmyAIMCMApplyHeadingStyle", rendered)
        self.assertIn("\\OhmyAIMCMApplyMathStyle", rendered)
    ...
    def test_hybrid_profile_drives_the_complete_visual_system(self) -> None:
        ...
        self.assertIn("\\OhmyAIMCMApplyRoleStyle{sans}{centered}{1.4500}", rendered)
        self.assertIn("\\OhmyAIMCMApplyNumbering{arabic}", rendered)
        self.assertIn("\\OhmyAIMCMApplyCaptionStyle{0.8400}{centering}{raggedright}", rendered)
    ...
    def test_paragraph_indent_token_survives_ctex_initialization(self) -> None:
        style = (SKILL_DIR / "assets" / "latex" / "OhmyAIMCM-style.sty").read_text(
            encoding="utf-8"
        )
        self.assertIn("\\renewcommand{\\OhmyAIMCMParagraphIndent}{#7}", style)
        self.assertIn(
            "\\AtBeginDocument{%\n  \\color{OhmyAIMCMInk}%\n"
            "  \\fontsize{\\OhmyAIMCMBodyFontSize pt}{\\OhmyAIMCMBodyLeading pt}\\selectfont\n"
            "  % ctexart initializes its paragraph layout at begin-document time.",
            style,
        )
        self.assertGreaterEqual(
            style.count("\\setlength{\\parindent}{\\OhmyAIMCMParagraphIndent em}"),
            2,
        )
