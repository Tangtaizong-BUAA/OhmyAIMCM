...
def _atomic_savefig(
    ...
    *,
    ...
    try:
        ...
        if format_name == "svg" and accessibility:
            namespace = "http://www.w3.org/2000/svg"
            ET.register_namespace("", namespace)
            ET.register_namespace("xlink", "http://www.w3.org/1999/xlink")
            tree = ET.parse(temporary)
            root = tree.getroot()
            for index, name in enumerate(("title", "desc")):
                element = root.find(f"{{{namespace}}}{name}")
                if element is None:
                    element = ET.Element(f"{{{namespace}}}{name}")
                    root.insert(index, element)
                element.text = accessibility[name]
...
def export_figure(
    ...
    *,
    ...
    if accessibility is not None and (
        ...
        or any(not isinstance(accessibility.get(k), str) or not accessibility[k].strip()
               for k in ("title", "desc"))
    ):
        raise CliError("accessibility requires nonempty title and desc strings")
    ...
    creator = user_metadata.setdefault(
        "Creator",
        f"lab-ultra-scientific-visualization figure_export.py; Matplotlib {mpl.__version__}",
    )
