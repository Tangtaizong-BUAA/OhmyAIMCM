...
DEFAULT_STYLE = {
    "theme": "paper",
    "font_family": 'Arial, Helvetica, "Noto Sans CJK SC", "Source Han Sans SC", "PingFang SC", "Microsoft YaHei", sans-serif',
    "font_size": 14,
    "bg_color": "#FFFFFF",
...
def validate_spec(spec: dict) -> list:
    ...
    # Canvas validation
    for dim in ("width", "height", "width_mm"):
        val = canvas.get(dim)
        if val is not None:
            if isinstance(val, bool) or not isinstance(val, (int, float)):
                issues.append(f"CRITICAL: canvas.{dim} must be a number")
            elif not math.isfinite(val) or val <= 0:
                issues.append(f"CRITICAL: canvas.{dim} must be positive and finite")

    # Style validation
    st = spec.get("style", {})
    family = st.get("font_family", DEFAULT_STYLE["font_family"])
    if not isinstance(family, str) or not family.strip():
        issues.append("CRITICAL: style.font_family must be a nonempty font-family string")
    fs = st.get("font_size")
    if fs is not None and (isinstance(fs, bool) or not isinstance(fs, (int, float)) or not math.isfinite(fs) or fs <= 0):
        issues.append(f"CRITICAL: style.font_size must be a positive number")
    if not isinstance(st.get("theme", "paper"), str) or st.get("theme", "paper") not in {"paper", "presentation"}:
        issues.append("CRITICAL: style.theme must be paper or presentation")
    pal = st.get("palette")
    if pal is not None:
    ...
    for i, node in enumerate(nodes):
        ...
        for coord in ("x", "y"):
            ...
            elif not math.isfinite(node[coord]):
                issues.append(f"CRITICAL: node '{nid}' {coord} must be finite")
        for dim in ("width", "height"):
            ...
            elif val is not None and (not math.isfinite(val) or val <= 0):
        ...
        if not isinstance(shape, str) or shape not in ALLOWED_SHAPES:
            issues.append(f"CRITICAL: node '{nid}' unknown shape '{shape}'")

    # Do not perform geometric arithmetic on malformed coordinates or sizes.
    ...
    for i, edge in enumerate(spec.get("edges", [])):
        ...
        if not isinstance(style, str):
            issues.append(f"CRITICAL: edge[{i}] style must be a string")
    ...
    for i, group in enumerate(spec.get("groups", [])):
        ...
        if not isinstance(node_ids_list, list):
            issues.append(f"CRITICAL: group[{i}] node_ids must be a list")
            continue
        for nid in node_ids_list:
            if not isinstance(nid, str) or nid not in node_ids:
                issues.append(f"CRITICAL: group[{i}] references unknown node '{nid}'")
    ...
    for i, edge in enumerate(spec.get("edges", [])):
        ...
        for field in ("thickness",):
            ...
            if val is not None and (isinstance(val, bool) or not isinstance(val, (int, float)) or not math.isfinite(val) or val <= 0):
                issues.append(f"CRITICAL: edge[{i}] {field} must be positive and finite")
    ...
    for i, group in enumerate(spec.get("groups", [])):
        ...
        if val is not None and (isinstance(val, bool) or not isinstance(val, (int, float)) or not math.isfinite(val) or val < 0):
            issues.append(f"CRITICAL: group[{i}] padding must be nonnegative and finite")
    ...
    for i, label in enumerate(spec.get("labels", [])):
        ...
        if anchor is not None and (not isinstance(anchor, str) or anchor not in ALLOWED_ANCHORS):
            issues.append(f"CRITICAL: label[{i}] unknown anchor '{anchor}'")
        for field in ("x", "y", "font_size"):
            val = label.get(field)
            if val is not None and (isinstance(val, bool) or not isinstance(val, (int, float)) or not math.isfinite(val) or (field == "font_size" and val <= 0)):
                issues.append(f"CRITICAL: label[{i}] {field} must be finite (positive for fonts)")
    ...
    for node in nodes:
        ...
        if nfs is not None and (isinstance(nfs, bool) or not isinstance(nfs, (int, float)) or not math.isfinite(nfs) or nfs <= 0):
            issues.append(f"CRITICAL: node '{nid}' font_size must be positive and finite")
    ...
    for node in nodes:
        n = {**DEFAULT_NODE, **node}
        font_size = n.get("font_size") or st.get("font_size", DEFAULT_STYLE["font_size"])
        lines = str(n.get("label", "")).replace("\\n", "\n").splitlines()
        if n.get("sublabel"):
            lines.append(str(n["sublabel"]))
        if any(estimate_text_width(line, font_size) > n["width"] - 12 for line in lines):
            issues.append(f"WARN: node '{n['id']}' estimated text width exceeds its box; inspect render")
        if len(lines) * (font_size + 2) > n["height"] - 8:
            issues.append(f"WARN: node '{n['id']}' estimated text height exceeds its box; inspect render")
        bound_w, bound_h = n["width"], n["height"]
        if n["shape"] == "circle":
            bound_w = bound_h = max(bound_w, bound_h)
        if n["x"] - bound_w / 2 < 0 or n["y"] - bound_h / 2 < 0 or n["x"] + bound_w / 2 > canvas.get("width", 800) or n["y"] + bound_h / 2 > canvas.get("height", 400):
            issues.append(f"CRITICAL: node '{n['id']}' extends outside canvas")
        if canvas.get("width_mm"):
            size_pt = font_size * canvas["width_mm"] / canvas.get("width", 800) * 72 / 25.4
            if size_pt < 7:
                issues.append(f"WARN: node '{n['id']}' text is {size_pt:.2f} pt at requested width; review readability")
...
def render_svg(spec: dict) -> str:
    ...
    critical = [issue for issue in validate_spec(spec) if issue.startswith("CRITICAL")]
    if critical:
    ...
    width_mm = canvas.get("width_mm")

    svg = Element("svg", {
        "xmlns": "http://www.w3.org/2000/svg",
        "viewBox": f"0 0 {width} {height}",
        "width": f"{width_mm}mm" if width_mm else str(width),
        "height": f"{height * width_mm / width:.8g}mm" if width_mm else str(height),
        "font-family": sanitize_text(style["font_family"]),
        "role": "img",
    })
    if spec.get("title"):
        SubElement(svg, "title").text = sanitize_text(spec["title"])
    if spec.get("description"):
        SubElement(svg, "desc").text = sanitize_text(spec["description"])
    ...
    for i, node in enumerate(spec.get("nodes", [])):
        n = {**DEFAULT_NODE, **node}
        if not paper and "shape" not in node:
            n["shape"] = "rounded"
        n["fill"] = sanitize_color(n.get("fill") or ("#FFFFFF" if paper else lighten_color(palette[i % len(palette)])))
        n["stroke"] = sanitize_color(n.get("stroke") or ("#30343B" if paper else palette[i % len(palette)]))
    ...
    for group in spec.get("groups", []):
        ...
        SubElement(svg, "rect", {
            ...
            "fill": sanitize_color(group.get("fill", "#FFFFFF" if paper else "#F5F5F5")),
            "stroke": sanitize_color(group.get("stroke", "#E0E0E0")),
            "stroke-width": "1", "rx": "0" if paper else "8",
        })
        if group.get("label"):
            lbl = SubElement(svg, "text", {
                "x": f"{min_x + 8:.1f}", "y": f"{min_y + 16:.1f}",
                "font-size": str(base_fs - 2),
                "fill": "#444444" if paper else "#999999", "font-weight": "bold",
            })
    ...
    for edge in spec.get("edges", []):
        ...
        if e.get("label"):
            ...
            SubElement(svg, "rect", {
                ...
                "fill": "#FFFFFF", "rx": "0" if paper else "3", "opacity": "1" if paper else "0.85",
            })
            lbl = SubElement(svg, "text", {
                "x": f"{lx:.1f}", "y": f"{ly + 2:.1f}",
                "font-size": str(base_fs - 3),
                "fill": "#444444" if paper else "#777777", "text-anchor": "middle",
            })
            lbl.text = label_text
    ...
    for node in spec.get("nodes", []):
        ...
        for li, line in enumerate(label_lines):
            lbl = SubElement(svg, "text", {
                ...
                "fill": n["text_color"],
                "text-anchor": "middle",
                "font-weight": "normal" if paper else "bold",
            })
            lbl.text = line
        ...
        if has_sub:
            sub = SubElement(svg, "text", {
                ...
                "font-size": str(fs - 3),
                "fill": "#444444" if paper else "#888888",
                "text-anchor": "middle",
            })
...
{
  "title": "string — accessible title (not visibly rendered)",
  "description": "string — accessible description",
  "canvas": {"width": number, "height": number, "width_mm": optional positive number},
  "style": {
    "theme": "paper (default) | presentation (legacy automatic colors/rounding)",
  ...
  "nodes": [{
    ...
    "fill": "#RRGGBB (paper default #FFFFFF; explicit values preserved)",
    "stroke": "#RRGGBB (paper default #30343B; explicit values preserved)",
...
def main():
    ...
    p_render.add_argument("--force", action="store_true", help="Explicitly replace an existing SVG")
    ...
    if args.command == "render":
        ...
        if Path(output).resolve() == Path(args.spec_file).resolve():
            parser.error("output cannot replace the source specification")
        if Path(output).is_symlink() or (Path(output).exists() and not args.force):
            parser.error("output exists or is a symlink; use a new path (or --force for a regular file)")
        if args.preview and (Path(output).with_suffix(".png").is_symlink() or (Path(output).with_suffix(".png").exists() and not args.force)):
            parser.error("preview exists; use a new path or --force")
        Path(output).parent.mkdir(parents=True, exist_ok=True)
        with open(output, "w" if args.force else "x", encoding="utf-8") as f:
            f.write(svg_content)
