# lab-ultra-scipilot-figure-skill

本文件保留上游 v2.1 介绍供历史参考。当前项目的有效流程见 [SKILL.md](SKILL.md)：样式、配方和导出已统一到 lab-ultra-scientific-figure-maker 的素材库；下文旧 setup/export 调用不再是默认入口。
...
科研工作者最大的画图痛点往往不是"不会用 matplotlib"，而是"手上一堆数据，不知道该用什么图把结论讲清楚"。`lab-ultra-scipilot-figure-skill` 是 SciPilot Skills 家族的第二个成员，专做这件事——
...
```
普通画图工具           lab-ultra-scipilot-figure-skill
─────────────         ─────────────────────
...
git clone https://github.com/Haojae/scipilot-figure-skill.git \
          ~/.claude/skills/lab-ultra-scipilot-figure-skill
pip install -r ~/.claude/skills/lab-ultra-scipilot-figure-skill/requirements.txt
```
...
| **lab-ultra-scipilot-figure-skill** | **v2.1.0 (本仓库)** | **可视化顾问 + 绘制 + 视觉自检闭环** |
...
The hardest part of scientific plotting is rarely "I don't know matplotlib" — it's "I have data, I don't know which chart conveys my conclusion." `lab-ultra-scipilot-figure-skill` is the second member of the SciPilot Skills family, built around this insight: **think first, plot second**.

### Why this isn't just a plotting tool

```
Generic plotter            lab-ultra-scipilot-figure-skill
──────────────             ──────────────────────
...
| **lab-ultra-scipilot-figure-skill** | **v2.1.0 (this repo)** | **Advisor + renderer + visual self-check loop** |
