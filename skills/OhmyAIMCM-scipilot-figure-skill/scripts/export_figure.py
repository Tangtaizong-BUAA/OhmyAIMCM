"""
OhmyAIMCM-scipilot-figure-skill :: export_figure.py
=========================================
...
def export_figure(
    ...
    for fmt in formats:
        if fmt in {"jpg", "jpeg"}:
            print(f"[OhmyAIMCM-scipilot-figure-skill] WARNING: skipping {fmt} — "
        ...
        saved.append(path)
        print(f"[OhmyAIMCM-scipilot-figure-skill] wrote {path}")
...
def _grayscale_from(fig, basename: str, dpi: int) -> str | None:
    ...
    try:
        ...
        print("[OhmyAIMCM-scipilot-figure-skill] Pillow not available; "
    ...
    print(f"[OhmyAIMCM-scipilot-figure-skill] wrote {gray_path} (grayscale preview)")
    return gray_path
...
def _cli() -> int:
    p = argparse.ArgumentParser(description="OhmyAIMCM-scipilot-figure-skill figure exporter")
