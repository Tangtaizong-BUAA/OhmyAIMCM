---
name: OhmyAIMCM-scientific-visualization
description: OhmyAIMCM CUMCM paper suite. Create and audit truthful, accessible, publication-ready scientific figures with Matplotlib, Seaborn, or Plotly. Use for figure design, multi-panel layouts, uncertainty and missing-data displays, color/contrast review, image metadata validation, and journal export planning.
license: MIT
allowed-tools: Read Write Edit Bash Glob Grep
metadata:
  version: "1.1"
  skill-author: K-Dense Inc.
  compatibility: Requires Python 3.11+ and uv for pinned examples. Bundled CLIs are network-free; Plotly static export with Kaleido v1 requires compatible Chrome or Chromium.
---
...
# OhmyAIMCM-scientific-visualization
...
In this project, use the shared [material library](../OhmyAIMCM-scientific-figure-maker/references/material-library.md) for upstream styles and recipe lookup. Resolve the sibling OhmyAIMCM-scientific-figure-maker/scripts directory onto sys.path, then use Matplotlib's object-oriented API and one temporary style context:
...
with publication_style("science", width_mm=89, height_mm=60):
    fig, ax = plt.subplots(
...
The existing style_presets.py remains a standalone fallback. Do not stack it, SciPilot setup_style, cnsplots setup, or Seaborn set_theme over a resolved publication_style context. Explicit request settings take priority over preset names.
...
report = export_figure(
    ...
    write_manifest=True,
    accessibility={"title": "Observed response", "desc": "Describe the actual evidence and uncertainty."},
)
```

The exporter refuses implicit overwrite, writes atomically, keeps vector DPI for embedded rasters, uses TIFF LZW, and can use PDF/PS Type 42 fonts. The optional accessibility argument writes root SVG title/desc elements. It does not validate scientific content or publisher acceptance.
