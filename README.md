...
Copyright 2026 OhmyAIMCM contributors.

本项目当前版本采用 [GNU General Public License v3.0](LICENSE)，许可证标识为 `GPL-3.0-only`。除另有标注外，该许可适用于本仓库贡献者拥有权利的原创代码、Skill、文档和论文案例内容。

对外分发本项目或受 GPL 覆盖的衍生作品时，须遵守 GPLv3 的许可、对应源码提供及声明保留等要求。单纯运行本工具生成论文，通常不会使生成的论文自动受 GPL 约束；若输出包含本项目受版权保护的内容，则需按具体内容判断。项目不提供许可证规定范围之外的担保，完整权利义务以许可证全文为准。

随附第三方组件、素材及引用内容的权利和许可保持原有归属。各目录中的第三方许可证、版权声明和来源记录继续适用，仓库级许可证不替代这些条款。

此前提交 `4c5103b` 曾以 Apache-2.0 发布。本次变更不撤销该版本已经授予的许可；当前版本及后续版本以各自随附的许可证为准。
